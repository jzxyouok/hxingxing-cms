...
class OperaRepository extends BaseRepository {
	...
	public function allpersons() {
		return $this->person->orderBy('uid','desc')->select('uid','name','fakeName','realMobile','fakeMobile','isPubed','company','position','otherName','otherMobile','remark')->get()->toArray();
	}
	...
	public function tags($data = []) {
		...
		$ret = Tags::select('category', DB::raw('GROUP_CONCAT(id) as ids,GROUP_CONCAT(code) as codes,GROUP_CONCAT(name) AS labels,GROUP_CONCAT(parentId) AS parentIds'))
			->groupBy('category')
			->get()->toArray();
